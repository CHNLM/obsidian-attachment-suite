...
- **下载即命名**：本地化下载成功后，若「命名」已开启，会自动接续统一命名（静默执行、不弹预览）
...
所有命令均可在命令面板中按中文关键词搜索（如「清理」「附件」「导出」「收集」「全库」「批量」「统一命名」「下载」）。
...
- **通用**：附件目录来源（跟随 Obsidian / 自定义）、通知级别（静默 / 仅摘要 / 详细；提示按级别分级停留时长：错误 5 秒、执行结果 3 秒、过程信息 2 秒，消息越长停留越久）
